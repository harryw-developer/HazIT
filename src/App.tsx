...
const APP_META: Record<string, { title: string; icon: ReactNode }> = {
  ...
  '/apps/enquiries': { title: 'Website Enquiries', icon: <EnquiriesIcon size={18} /> },
}
  ...
  return (
    <div className="app-shell min-h-screen px-3 pb-14">
      <Routes>
        ...
        <Route path="/apps/enquiries" element={win('Website Enquiries', <EnquiriesIcon size={18} />, <EnquiriesPage />)} />
      </Routes>
      <Taskbar />
    </div>
  )
...
// Public marketing website — always available, signed in or not.
function PublicSite() {
  return (
    <SiteLayout>
      <Routes>
...
function RootRouter() {
  ...
  // Marketing pages are public and never gated
  if (SITE_PATHS.includes(pathname)) return <PublicSite />
  ...
  // Signed out: "/" is the website, "/portal" (and anything else) is the login
  if (!session) {
    if (pathname === '/') return <PublicSite />
    return <LoginPage />
  }

  // Signed in but landing on the login link — send them into the app
  if (pathname === '/portal') return <Navigate to="/" replace />
